...
supabase/migrations/20260516010000_uploads_kyc_and_loan_duration_rules.sql
```
...
Receipt uploads are stored in the private `receipts` bucket. The browser sends the image to `/api/uploads/private-image` with the user's bearer token; the server verifies the user and uploads with the server-only Supabase client. The app accepts image files up to 5MB, stores private object paths, and admins view receipts through short-lived signed URLs generated by `/api/uploads/signed-url`.

## 6. Registration Deposit And Onboarding Credit

Users complete registration with Supabase Auth, then pay a ₦1,000 registration deposit to the platform account details.
The app records their payment proof as pending. Once an admin approves it, the app records the payment reference and credits a ₦2,000 onboarding credit to their wallet.
That ₦2,000 is not a loan, is not shown in the loan list, and does not need repayment. It becomes withdrawable after KYC is completed.
...
KYC unlocks only after the registration deposit is confirmed. Passport uploads use the same server-backed private upload route and are stored in the private `kyc-documents` bucket with the same 5MB image limit.

## 7. Withdrawal Rule

Withdrawals require a confirmed registration deposit and completed KYC.

When a user has an active platform loan, the wallet must contain:

```text
withdrawal amount + 50% of the active platform loan amount
```

Only the withdrawal amount is deducted. The 50% platform loan condition remains in the user's wallet balance after the withdrawal. The ₦2,000 onboarding credit is not a platform loan and has no repayment or retained-wallet condition.

## 8. Platform Loan Rules

The MVP platform loan flow is enforced in Postgres through `me2u_request_platform_loan`:

- Platform loans start from ₦10,000.
- A user must have a confirmed registration deposit and completed KYC before requesting a platform loan.
- The user's wallet must already contain 50% of the requested loan amount.
...
- New loans have a maximum duration of 14 days.
...
7. Receiving a ₦2,000 onboarding credit after admin approval
8. Completing KYC before withdrawing or requesting platform loans
...
- `/api/uploads/private-image` accepts authenticated image uploads for both `receipts` and `kyc-documents`, stores private object paths, and rejects non-image or larger-than-5MB files.
- `/api/uploads/signed-url` returns receipt links only for admin users.
...
- Admin approval of a registration deposit sets `registration_deposit_paid=true`, stores `registration_payment_reference`, credits the ₦2,000 onboarding credit without creating a loan, and credits any direct referral reward only once.
- KYC is unavailable before the confirmed registration deposit, then available from the dashboard.
- New marketplace and platform loans use a 1-14 day duration, and repayments work immediately after disbursement.
