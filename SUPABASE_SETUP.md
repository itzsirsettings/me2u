...
The app records their payment proof as pending. Once an admin approves it, the app records the payment reference.
The registration deposit is used to fund the platform's balance sheet.
...
Only the withdrawal amount is deducted. The 50% platform loan condition remains in the user's wallet balance after the withdrawal.

## 8. Platform Loan Rules

The MVP platform loan flow is enforced in Postgres through `me2u_request_platform_loan`:

- Platform loans start from ₦5,000.
...
Users can share their username as a referral code. When a direct referral completes their KYC and their first loan repayment, the referrer earns a reward:

```text
₦500 affiliate reward
```

The reward is credited automatically to the referrer's wallet after the referee's first successful repayment.
...
7. Completing KYC after deposit is approved
8. Requesting platform loans or using the marketplace
9. Referrer gets reward after KYC and first loan repayment
...
- Admin approval of a registration deposit sets `registration_deposit_paid=true` and stores `registration_payment_reference`.
