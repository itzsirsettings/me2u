...
import { Gift, TrendingUp, Users, Calendar, CheckCircle } from "lucide-react";
...
export default function CircleRewardsTracker({ circleId }: CircleRewardsTrackerProps) {
  ...
  useEffect(() => {
    const loadRewards = async () => {
      try {
        const response = await fetch(`/api/circles/${circleId}/rewards`, {
          cache: "no-store",
        });
        ...
        console.error("Failed to load circle rewards:", error);
      } finally {
        setLoading(false);
      }
    };
