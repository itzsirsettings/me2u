  ...
  return [
    {
      ...
      detail: "Total amount borrowed across all loans, tracked in real-time.",
    },
    {
      label: "Successfully Repaid",
      value: formatMetric(metrics.totalRepaid, "money"),
      detail: "Total loan repayments completed by our trusted community.",
    },
    {
      label: "Active Circles",
      value: formatMetric(metrics.activeCircles),
      detail: "Community lending circles pooling resources together.",
    },
    {
      label: "Trusted Users",
      value: formatMetric(metrics.totalUsers),
      detail: "Registered users building financial trust together.",
    },
    {
      label: "Completed Loans",
      value: formatMetric(metrics.successfulLoans),
      detail: "Total loans successfully repaid to date.",
    },
    {
      label: "Active Loans",
      value: formatMetric(metrics.activeLoans),
      detail: "Currently active loans being serviced.",
    },
    {
      label: "Average Trust Score",
      value: `${Math.round(metrics.trustScoreAvg)}/100`,
      detail: "Community trust average across verified users.",
    },
  ];
}
      ...
      fetch("/api/platform/stats", { cache: "no-store" })
        ...
        .then((data) => {
          if (!cancelled && data.ok && data.stats) {
            setMetrics({ ...defaultMetrics, ...data.stats });
            setLoaded(true);
          }
        })
        .catch(() => {
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="mb-40">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground font-medium leading-relaxed max-w-2xl">
              Hear from verified Me2U users who have successfully borrowed and repaid interest-free loans.
            </p>
          </motion.div>
          <SuccessStoryCarousel />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          >
            <GlowCard customSize glowColor="green" className="h-full rounded-[3rem] border border-[var(--color-border)] bg-[var(--landing-proof-card)] p-12 text-card-foreground shadow-[0_1rem_2rem_-1rem_rgba(8,19,32,0.05)] transition-colors duration-500 hover:bg-card md:p-16">
              <h2 className="text-3xl md:text-4xl font-black mb-8 leading-tight">Trust built on <br/><span className="text-[var(--landing-accent-strong)]">transparency.</span></h2>
              <p className="text-lg text-muted-foreground font-medium leading-relaxed mb-0">
                Every number you see is pulled from our live production database. No inflated metrics, no estimates—just real data from real loans, real repayments, and real community growth.
              </p>
            </GlowCard>
          </motion.div>
