...
export default function Loans() {
  ...
  const platformLoans = visibleLoans.filter(
  ...
  const handleRequestLoan = async () => {
    ...
    toast.success(
      `Loan of ₦${repeatLoanAmount.toLocaleString()} has been added to your wallet.`,
    );
  };
  ...
  return (
    <motion.div
      className="app-mobile-screen mx-auto w-full max-w-md px-3.5 pt-[3.85rem] md:max-w-4xl md:px-6 md:py-24"
    ...
    >
      <motion.div
        ...
        className="mb-4 flex min-w-0 flex-col gap-3 md:mb-12 md:flex-row md:items-end md:justify-between"
      >
        <div className="min-w-0">
          <p className="mb-2 hidden items-center gap-2 text-sm font-sans font-bold uppercase tracking-wider text-[var(--color-text-secondary)] md:flex">
            <Me2uIcon name="requestMoney" size={18} />
            Credit
          </p>
        ...
        <div className="mobile-soft-card min-w-0 rounded-[20px] border border-[var(--color-border)] bg-[var(--color-bg-card)] px-3.5 py-2.5 shadow-[2px_2px_0px_var(--color-shadow)] md:px-5 md:py-4">
          <p className="text-xs font-sans font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">
            Wallet Balance
          </p>
          <p className="overflow-anywhere text-xl font-display leading-none md:text-2xl">
          ...
          <Card className="kinetic-border bg-[var(--color-bg-card)] p-5 shadow-[4px_4px_0px_var(--color-shadow)] md:p-6">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="rounded-[5px] bg-[var(--color-bg-secondary)] p-3">
                ...
                <p className="mt-1 font-mono text-lg text-[var(--color-text-primary)]">
                  {Math.round(currentDepositRate * 100)}%
                </p>
              </div>
              <div className="rounded-[5px] bg-[var(--color-bg-secondary)] p-3">
                <p className="text-xs font-sans font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="grid min-w-0 gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
                ...
                {!requestDisabled ? (
                  <LoadingButton
                    ...
                    icon={<Me2uIcon name="cash" size={20} />}
              ...
              {repeatAmountIsValid && (
                <div className="mt-5 grid gap-3 rounded-[5px] border border-dashed border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-4 text-sm md:grid-cols-3">
                  <div className="min-w-0">
                    <p className="font-sans font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">
                      Minimum
                    </p>
                    <p className="overflow-anywhere mt-1 font-mono text-lg text-[var(--color-text-primary)]">
                      ₦{repeatPlatformLoanMinimum.toLocaleString()}
                    </p>
                  </div>
                  <div className="min-w-0">
                    <p className="font-sans font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">
                      Security Deposit ({Math.round(currentDepositRate * 100)}%)
                    </p>
                    <p className="overflow-anywhere mt-1 font-mono text-lg text-[var(--color-text-primary)]">
                  ...
                  <div className="min-w-0">
                    ...
                    <p
                      className={`mt-1 font-sans font-semibold ${requestDisabled ? "text-[var(--color-warning-text)]" : "text-[var(--color-positive-text)]"}`}
                    >
                      {activePlatformLoan
                        ? "Active loan pending"
                        : !user?.registrationDepositPaid
              ...
              {securityDeposit > 0 && repeatAmountIsValid && (
                <p className="mt-3 text-xs text-[var(--color-text-secondary)]">
                  ₦{securityDeposit.toLocaleString()} will be held as a security deposit and
            ...
            >
              <Card className="kinetic-border p-5 shadow-[4px_4px_0px_var(--color-shadow)] bg-[var(--color-bg-card)] md:p-8">
                <div className="mb-4 flex min-w-0 flex-col gap-2 border-b border-[var(--color-border)] pb-4 sm:flex-row sm:items-center sm:justify-between">
                  <p
                    className={`min-w-0 font-sans font-semibold ${loan.status === "active" ? "text-[var(--color-accent-primary)]" : "text-[var(--color-text-secondary)]"}`}
                  >
                ...
                <div className="flex min-w-0 flex-col justify-between gap-4 md:flex-row md:items-end md:gap-6">
                  <div className="min-w-0">
                    <p className="overflow-anywhere text-2xl font-display leading-none md:text-5xl">
                    ...
                    {loan.securityDeposit > 0 && loan.role === "borrower" && (
                      <p className="mt-2 text-sm font-sans">
                        ...
                        <span className="font-semibold text-[var(--color-accent-primary)]">
                          ₦{loan.securityDeposit.toLocaleString()} held
                        </span>
                  ...
                  {loan.status === "active" && loan.role === "borrower" && (
                    <LoadingButton
                      ...
                      successText="Repaid!"
                      onClick={() =>
                        handleRepay(loan.id, loan.amount, loan.rate, loan.securityDeposit)
                      }
                    />
                  )}
                </div>
