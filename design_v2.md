...
| `--rose` | `#F43F5E` | Danger/error, destructive actions, debit amounts |
...
:root {
  ...
  --color-danger:         #F43F5E;
  --color-danger-bg:      color-mix(in srgb, var(--color-danger) 12%, var(--snow));
  --color-danger-text:    color-mix(in srgb, var(--color-danger) 74%, var(--navy));
  --color-danger-hover:   color-mix(in srgb, var(--color-danger) 14%, var(--snow));
  --color-text-danger:    var(--color-danger-text);
  --color-negative-bg:    var(--color-danger-bg);
  --color-negative-text:  var(--color-danger-text);
}
```
...
Use **`Me2uIcon`** (`components/Me2uIcon.tsx`) — a self-contained set of **44 hand-authored filled 24×24 glyphs**. No remote icon dependency (Icons8/Tabler removed from app code).

```tsx
import Me2uIcon from "@/components/Me2uIcon";

<Me2uIcon name="wallet" size={24} className="text-[var(--color-accent-primary)]" />
```

| Prop | Type | Default | Purpose |
|---|---|---|---|
| `name` | `Me2uIconName` | — | One of the 44 glyph keys below |
| `size` | `number` | `24` | Width/height in px |
| `className` | `string` | — | Color via `currentColor`, layout |
| `label` | `string` | — | Visible text label if the icon is standalone |
| `decorative` | `boolean` | `false` | Hides the icon from the a11y tree |

| Context | Size | Color |
|---|---|---|
| Nav / feature icons | 20–24px | `currentColor` |
| Inline with text | 16px | inherit |
| Action buttons | 16–18px | inherit |

Glyph keys (`Me2uIconName`): `app, home, wallet, market, loans, profile, sun, moon, shield, email, phone, lock, visible, invisible, referral, moneyBag, requestMoney, bank, cash, tap, check, back, globe, certificate, group, bill, savings, deal, family, receipt, star, qr, trophy, medal, book, security, fingerprint, key, alert, freeze, download, mobile, bell, support`.

```html
<Me2uIcon name="swap/transfer">  <!-- P2P / transfer -->
<Me2uIcon name="wallet">         <!-- Wallet -->
<Me2uIcon name="star">           <!-- Trust score -->
<Me2uIcon name="referral">       <!-- Referral -->
<Me2uIcon name="security">       <!-- Security / compliance -->
<Me2uIcon name="key">            <!-- Lock / collateral -->
<Me2uIcon name="check">          <!-- Verified / done -->
<Me2uIcon name="bell">           <!-- Notifications -->
<Me2uIcon name="back">           <!-- Back arrow -->
```
